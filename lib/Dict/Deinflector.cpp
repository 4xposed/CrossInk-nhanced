...
// Japanese deinflection rules, linguistic facts covering common verb/adjective conjugation patterns.
// Organized by conjugation type.
...
static_assert(sizeof(kRules) / sizeof(kRules[0]) == Deinflector::kRuleCount);

bool sameText(const DeinflectionCandidate& candidate, const char* text, size_t length) {
  return candidate.byteLength == length && std::memcmp(candidate.text, text, length) == 0;
}
...
void deinflectWithRules(std::string_view surface, const RuleType* rules, size_t ruleCount, DeinflectionBuffer& out) {
  ...
  DeinflectionCandidate& first = out.candidates[out.count++];
  std::memcpy(first.text, surface.data(), surface.size());
  first.byteLength = static_cast<uint8_t>(surface.size());
  first.condition = WordCondition::DICT;
  ...
  for (uint8_t candidateIndex = 0; candidateIndex < out.count && out.count < out.kCapacity; ++candidateIndex) {
    ...
    for (size_t ruleIndex = 0; ruleIndex < ruleCount; ++ruleIndex) {
      ...
      if (current.byteLength < fromLength ||
          std::memcmp(current.text + current.byteLength - fromLength, rule.from, fromLength) != 0) {
        continue;
      }
      const size_t stemLength = current.byteLength - fromLength;
      const size_t toLength = std::strlen(rule.to);
      const size_t newLength = stemLength + toLength;
      if (newLength >= DictIndexRecord::HEADWORD_SIZE) continue;

      char next[DictIndexRecord::HEADWORD_SIZE]{};
      std::memcpy(next, current.text, stemLength);
      std::memcpy(next + stemLength, rule.to, toLength);
      bool duplicate = false;
      for (uint8_t existing = 0; existing < out.count; ++existing) {
        if (sameText(out.candidates[existing], next, newLength)) {
          duplicate = true;
          break;
        }
      }
      ...
      DeinflectionCandidate& added = out.candidates[out.count++];
      std::memcpy(added.text, next, newLength);
      added.byteLength = static_cast<uint8_t>(newLength);
      added.condition = rule.condOut;
      if (out.count == out.kCapacity) break;
    }
  }
}
...
void Deinflector::deinflectForTest(std::string_view surface, const DeinflectionRuleForTest* rules, size_t ruleCount,
                                   DeinflectionBuffer& out) {
  deinflectWithRules(surface, rules, ruleCount, out);
}
#endif
