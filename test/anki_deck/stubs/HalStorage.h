...
class HalFile {
 public:
  ...
  bool sync() { stream_.flush(); return static_cast<bool>(stream_); }
  void flush() { stream_.flush(); }
  bool close() { if (stream_.is_open()) stream_.close(); return true; }
...
class HalStorage {
 public:
  static HalStorage& getInstance() { static HalStorage instance; return instance; }
  ...
  bool ensureDirectoryExists(const char* path) { std::error_code error; std::filesystem::create_directories(resolve(path), error); return !error; }
  bool exists(const char* path) { return std::filesystem::exists(resolve(path)); }
  bool remove(const char* path) { std::error_code error; return std::filesystem::remove(resolve(path), error); }
  bool rename(const char* oldPath, const char* newPath) { std::error_code error; std::filesystem::rename(resolve(oldPath), resolve(newPath), error); return !error; }
  ...
  bool openFileForWrite(const char*, const std::string& path, HalFile& file) { return file.open(resolve(path.c_str()), true); }
