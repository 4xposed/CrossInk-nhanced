...
int bookWriteOpens = 0;
int growOnReadOpenCall = 0;
bool emulate32BitSize = false;
int openReadCalls = 0, failOpenReadCall = 0, readCalls = 0, failReadCall = 0;
...
std::string mapped(const char* p) {
  return (!strncmp(p, "/.crosspoint", 12) || !strncmp(p, "/.crossink-stats-backup", 22)) ? root + std::string(p) : p;
}
}  // namespace storage_test
...
int HalFile::read(void* d, size_t n) {
  if (++storage_test::readCalls == storage_test::failReadCall) return -1;
  return file_ ? int(fread(d, 1, n, file_)) : -1;
}
int HalFile::read() { return file_ ? fgetc(file_) : -1; }
size_t HalFile::write(const void* d, size_t n) {
  if (!storage_test::failWritePath.empty() && path_.find(storage_test::failWritePath) != std::string::npos) return 0;
...
bool HalStorage::openFileForRead(const char*, const char* p, HalFile& f) {
  if (++storage_test::openReadCalls == storage_test::failOpenReadCall) return false;
  if (storage_test::openReadCalls == storage_test::growOnReadOpenCall) {
    const auto path = storage_test::mapped(p);
    std::filesystem::resize_file(path, (uint64_t(1) << 32) + std::filesystem::file_size(path));
  }
  f = open(p);
  return bool(f);
}
bool HalStorage::openFileForWrite(const char*, const char* p, HalFile& f) {
  if (std::string(p).find("/book/") != std::string::npos) ++storage_test::bookWriteOpens;
...
std::vector<std::string> HalStorage::listFiles(const char* path, int maxFiles) {
  ...
  for (const auto& entry : std::filesystem::directory_iterator(storage_test::mapped(path), error)) {
    if (static_cast<int>(names.size()) == maxFiles) break;
